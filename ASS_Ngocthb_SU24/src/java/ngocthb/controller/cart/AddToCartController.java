...
public class AddToCartController extends HttpServlet {
   ...
   protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int quantity = Integer.parseInt(request.getParameter("commerce-add-to-cart-quantity-input"));
            HttpSession sesssion = request.getSession();
            Cart cart = (Cart) sesssion.getAttribute("CART");
            if (cart == null) {
                cart = new Cart();
            }
            Course course = new Course(crs.getId(), crs.getCoursename(), crs.getCategoryid(), crs.getDescription(), crs.getImages(), crs.getStartdate(), crs.getEnddate(), crs.getPrice(), crs.isStatus(), quantity);
            boolean check = cart.addCart(course);
